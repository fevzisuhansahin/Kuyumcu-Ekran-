...
let ziynetUrunleri = [
    ...
    { isim: "ÇEYREK", kod: "CEYREK", yeniAlis: 1.63, yeniSatis: 1.65, eskiAlis: 1.625, eskiSatis: 1.645, alisAyar: 0, satisAyar: 50 },
    { isim: "YARIM", kod: "YARIM", yeniAlis: 3.27, yeniSatis: 3.29, eskiAlis: 3.24, eskiSatis: 3.275, alisAyar: 0, satisAyar: 50 },
    { isim: "TAM", kod: "TAM", yeniAlis: 6.51, yeniSatis: 6.54, eskiAlis: 6.47, eskiSatis: 6.525, alisAyar: 0, satisAyar: 50 },
    { isim: "GREMSE", kod: "GREMSE", yeniAlis: 16.22, yeniSatis: 16.30, eskiAlis: 16.14, eskiSatis: 16.26, alisAyar: 0, satisAyar: 50 },
    { isim: "ATA", kod: "ATA", yeniAlis: 6.67, yeniSatis: 6.715, eskiAlis: 6.67, eskiSatis: 6.715, alisAyar: 0, satisAyar: 50 }
];

let gramUrunleri = [
    { isim: "1 GR", kod: "KULCEALTIN", alisAyar: 0, satisAyar: 50 }
];

let piyasaUrunleri = [
    { isim: "ALTIN", kod: "ALTIN", alisAyar: 0, satisAyar: 50 },
    { isim: "GÜMÜŞ", kod: "GUMUSTRY", alisAyar: 0, satisAyar: 50 }
];
...
function fiyatlariHesaplaVeYayinla(yeniAnaVeri) {
    ...
    ziynetUrunleri.forEach(ayar => {
        tvVerisi["Ziynet & Sarrafiye"].push({
            ...
            yeniAlis: Math.floor(hasAltinAlis * ayar.yeniAlis) + ayar.alisAyar,
            yeniSatis: Math.ceil(hasAltinSatis * ayar.yeniSatis) + ayar.satisAyar,
            eskiAlis: Math.floor(hasAltinAlis * ayar.eskiAlis) + ayar.alisAyar,
            eskiSatis: Math.ceil(hasAltinSatis * ayar.eskiSatis) + ayar.satisAyar
        });
    });
    ...
    gramUrunleri.forEach(ayar => {
        if (anaVeri[ayar.kod]) {
            tvVerisi["Gram Altın"].push({
                ...
                alis: Math.floor(anaVeri[ayar.kod].alis) + ayar.alisAyar,
                satis: Math.ceil(anaVeri[ayar.kod].satis) + ayar.satisAyar
            });
        }
    });
